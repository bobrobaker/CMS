...
The store is its own Dolt repository inside the git repo; git ignores `.dolt/` and
carries only `monition/dump.sql`, regenerated by pre-commit (a derived view — never
hand-edit it). Snapshot the store after mining (`monition commit`); use `dolt log`
/ `dolt diff` inside `monition/` for data history. Ad-hoc `rate`/`log-recurrence`
during a session leave uncommitted store state too — the pre-commit hook folds
`monition/dump.sql` into whatever you commit next, so snapshot those ratings on
their own first if you want them out of an unrelated commit's diff. Fresh clone without `.dolt/`:
`cd monition && dolt init && dolt sql < dump.sql`. The `dolt` binary lives on PATH
